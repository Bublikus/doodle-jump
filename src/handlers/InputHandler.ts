type InputHandlerActions = Partial<TouchGestureHandlerActions & DesktopInputHandlerActions>
...
export class TouchGestureHandler {
  ...
  private handleTouchMove(event: TouchEvent) {
    ...
    if (isSwiping) {
      ...
      const swipeDirection = this.getSwipeDirection(this.swipeStartX, this.swipeStartY, touchMoveX, touchMoveY)
  ...
  private handleTouchEnd(event: TouchEvent) {
    ...
    const isHold = Math.abs(touchEndX - this.touchStartX) < 10 && Math.abs(touchEndY - this.touchStartY) < 10

    this.lastTapTime = Date.now()

    if (isDoubleTapHandler && timeSinceLastTap <= this.doubleClickDuration && isHold) {
  ...
  private getSwipeDirection(x1: number, y1: number, x2: number, y2: number): TouchActionKeys {
    const deltaX = x2 - x1
    const deltaY = y2 - y1
...
export type DesktopInputHandlerActions = Partial<Record<DesktopActionKeys, (e: KeyboardEvent | MouseEvent) => void>>
...
export class DesktopInputHandler {
  ...
  private handleMouseEvents(event: MouseEvent): void {
    ...
    switch (event.type) {
      ...
      case 'mouseup':
        if (event.button === 0) {
          clearTimeout(this.longClickTimeout)
          if (Date.now() - this.lastClickTime < this.longClickDuration && actionKey !== 'doubleClick') {
            actionKey = 'click'
          }
        }
        break
  ...
  private handleScroll(event: WheelEvent): void {
    ...
    const scrollTop = this.element instanceof Document ? document.documentElement.scrollTop : this.element.scrollTop
    const scrollLeft = this.element instanceof Document ? document.documentElement.scrollLeft : this.element.scrollLeft
